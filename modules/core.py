...
class Core:
	def __init__(self):
		...
		logging.basicConfig(format=f"[%(levelname)s] [%(asctime)s] [%(name)s]: %(message)s", level=logging.__getattribute__(self.config.logLevel), handlers=[logging.FileHandler(self.config.logFile), logging.StreamHandler()])
